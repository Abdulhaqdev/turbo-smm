...
import { ChevronLeft, ChevronRight, Edit, Mail, Phone, User } from "lucide-react";
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // const transactionsResponse = await apiService.get<{ results: Transaction[] }>(
        //   `/api/transactions/?limit=5`
        // );
        // if (transactionsResponse.status === 200 && transactionsResponse.data?.results) {
        //   setRecentTransactions(transactionsResponse.data.results);
        // } else {
        //   console.log("Failed to fetch transactions:", transactionsResponse.error);
        // }

        // Fetch recent orders
        const ordersResponse = await apiService.get<{ results: Order[] }>(`/api/orders/?limit=5`);
  ...
  const handleSaveProfile = async () => {
    ...
    if (updateResponse.status === 200 && updateResponse.data) {
      ...
      toast({
        title: "Update failed",
        description: updateResponse.error?.general?.[0] || "Failed to update profile. Please try again.",
        variant: "destructive",
      });
    }
  };

  const joinDateDisplay = isValidDate(userProfile?.joinDate) ? formatShortDate(userProfile?.joinDate) : "N/A";
      ...
      <main className="flex-1 p-4 md:p-6">
        <div className="mx-auto max-w-6xl">
                ...
                <div className="flex flex-col gap-6 md:flex-row">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Member since {joinDateDisplay}</p>
                      <div className="mt-2 flex items-center gap-2">
                        <span className="text-sm font-medium">Balance:</span>
                        <span className="font-semibold text-primary">
                          ${(parseFloat(userProfile?.balance || "0") || 0).toFixed(2)}
                        </span>
                        <Button variant="outline" size="sm" onClick={() => router.push(ROUTES.ADD_FUNDS)}>
                          Add Funds
                        </Button>
                      </div>
                    </div>
                    ...
                    {isEditing ? (
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-1">
                          ...
                          <p className="font-medium">{userProfile?.first_name || "Not provided"}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-muted-foreground">Last Name</p>
                          <p className="font-medium">{userProfile?.last_name || "Not provided"}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-sm text-muted-foreground">Username</p>
                          <p className="font-medium">{userProfile?.username || "Not provided"}</p>
                        </div>
                        <div className="space-y-1">
          ...
          <section className="mb-6 relative">
            ...
            <ScrollArea className="pb-4" ref={featuresScrollRef} onScroll={checkScrollable}>
              <div className="flex gap-4 pb-2">
                <Card className="min-w-[300px] max-w-[300px]">
                      ...
                      <div className="space-y-3">
                        {recentOrders.map((order) => (
                          <div key={order.id} className="flex items-center justify-between border-b pb-2 last:border-0">
                            <div>
                              <p className="font-medium truncate max-w-[180px]">{order.id.substring(0, 8)}</p>
                              <p className="text-xs text-muted-foreground">
                                {isValidDate(order.date) ? formatShortDate(order.date) : "N/A"}
                              </p>
                            </div>
                  ...
                  <CardFooter>
                    <Button variant="outline" className="w-full" onClick={() => router.push(ROUTES.ORDERS)}>
                      View All Orders
                    </Button>
                  </CardFooter>
                </Card>
                ...
                <Card className="min-w-[300px] max-w-[300px]">
                        ...
                        {recentTransactions.map((transaction) => (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">
                                {isValidDate(transaction.date) ? formatShortDate(transaction.date) : "N/A"} •{" "}
                                {transaction.type}
                              </p>
                            </div>
                  ...
                  <CardFooter>
                    <Button variant="outline" className="w-full" onClick={() => router.push("/account/transactions")}>
                ...
                <Card className="min-w-[300px] max-w-[300px]">
                  ...
                  <CardContent className="h-[200px] flex flex-col justify-center items-center gap-4 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Add funds to your account to place orders</p>
                  </CardContent>
                  <CardFooter>
          ...
          <section>
            <Card>
              <CardHeader>
                ...
                <CardDescription>View your recent activity on TurboSMM</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    {recentOrders.length > 0 ? (
                      <>
                          ...
                          >
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {isValidDate(order.date) ? formatDateTime(order.date) : "N/A"} • $
                        ...
                        <div className="flex justify-center pt-2">
                          <Button variant="outline" onClick={() => router.push(ROUTES.ORDERS)}>
                            View All Orders
                          </Button>
                        </div>
                      </>
                      ...
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <Button onClick={() => router.push(ROUTES.NEW_ORDER)}>Place an Order</Button>
                      </div>
                    )}
                  </TabsContent>
                  <TabsContent value="transactions" className="space-y-4">
                    {recentTransactions.length > 0 ? (
                        ...
                        {recentTransactions.map((transaction) => (
                          ...
                          >
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {isValidDate(transaction.date) ? formatDateTime(transaction.date) : "N/A"}
                              </p>
                            </div>
                            <div className="flex items-center gap-4">
                              <p className={`font-semibold ${transaction.type === "deposit" ? "text-green-500" : ""}`}>
                      ...
                      <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <p className="mb-6 text-muted-foreground">
                          You haven't made any transactions yet. Add funds to get started!
                        </p>
                        <Button onClick={() => router.push(ROUTES.ADD_FUNDS)}>Add Funds</Button>
                      </div>
                    )}
                  </TabsContent>
                </Tabs>
